  ...
  void initState() {
    ...
    widget.databaseReference!
      ...
      if(event.snapshot.value != null) {
        setState(() { reviews.add(Review.fromSnapshot(event.snapshot));
        });
      }
    });
      ...
      body: CustomScrollView(
        slivers: <Widget>[
          ...
          SliverList(
              delegate: SliverChildListDelegate([
                ...
                Center(
                  child: Column(
                    children: <Widget>[
                      Hero( tag: 'tourinfo${widget.index}',
                        child: Container( width: 300.0,
                          height: 300.0,
                          decoration: BoxDecoration( shape: BoxShape.circle,
                            border: Border.all(color: Colors.black, width: 1),
                            image: DecorationImage( fit: BoxFit.fill, image: getImage(widget.tourData!.imagePath) ,),
                          ),
                        ),
                      ),
                      Padding( padding: const EdgeInsets.only(top: 20, bottom: 20),
                        child: Text( widget.tourData!.address!, style: const TextStyle(fontSize: 20), ),
                      ),
                      getGoogleMap(),
          ...
          SliverPersistentHeader(
            pinned: true,
            delegate: _HeaderDelegate( minHeight: 50,
              maxHeight: 100,
              ...
              return Card(
                child: InkWell(
                  child: Padding(
                    ...
                    child: Text( '${reviews[index].id} : ${reviews[index].review}', style: const TextStyle(fontSize: 15), ),
                  ),
                  onDoubleTap: (){
                    if(reviews[index].id == widget.id) {
                      widget.databaseReference!
                        ...
                        .remove();
                      setState(() { reviews.removeAt(index);
                      });
                    }
                  },
                ),
            ...
            delegate: SliverChildListDelegate([
              MaterialButton( onPressed: () {
                showDialog( context: context,
                  builder: (context) {
                    return AlertDialog( title: const Text('후기 쓰기'), content: TextField( controller: _reviewTextController,),
                      actions: <Widget>[
                        MaterialButton( onPressed: () {
  ...
  getDisableInfo() {
    widget.databaseReference!
      ...
      if (_disableInfo!.id == null) {
        setState(() { _disableWidget = false;
        });
      } else {
        setState(() { _disableWidget = true;
        });
      }
    });
  }
