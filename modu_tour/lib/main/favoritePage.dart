...
class _FavoritePage extends State<FavoritePage> {
      ...
      body: Center(
        child: FutureBuilder( builder: (context, snapshot) {
            switch (snapshot.connectionState) {
              case ConnectionState.none: return const CircularProgressIndicator();
                              ...
                              SizedBox(
                                ...
                                child: Column(
                                  ...
                                  children: <Widget>[
                                    ...
                                    Text('주소 : ${info.address}'), info.tel != 'null' ? Text('전화 번호 : ${info.tel}') : Container(),
                                  ],
                                ),
                              )
                            ],
  ...
  void deleteTour(Future<Database> db, TourData info) async {
    ...
    await database.delete('place', where: 'title=?', whereArgs: [info.title]).then((value){
      setState(() { _tourList = getTodos(); });
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: const Text('즐겨찾기를 해제합니다')));
    });
  }
